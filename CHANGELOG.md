...
- **Album art now renders inside tmux on Ghostty** (and other Kitty-graphics
  terminals). fuga's in-tmux Kitty-capability probe could race to a false
  negative and drop to text-only halfblocks even when the terminal fully
  supports Kitty graphics. It now also recognizes Ghostty, kitty, and WezTerm by
  environment variables that survive into tmux, so the art no longer disappears
  when running under a multiplexer.
